...
public class AddFragment extends Fragment {
    ...
    // CHANGED: ToiletRepository now handles all Firestore calls — SRP applied.
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        btnAdd.setOnClickListener(v -> {
            try {
                ...
                // CHANGED: Using setters instead of direct field access — encapsulation applied.
                ...
                // CHANGED: ChipUtils.getSelectedTags replaces the duplicated loop
                List<String> selectedTags = ChipUtils.getSelectedTags(cgTags);
                toilet.setTags(selectedTags);

                // CHANGED: repository.addToilet() instead of calling Firestore directly
                repository.addToilet(
                        toilet,
                        unused -> Toast.makeText(getContext(), "Toilet added successfully!", Toast.LENGTH_SHORT).show(),
                        e -> Toast.makeText(getContext(), "Error adding toilet: " + e.getMessage(), Toast.LENGTH_LONG).show()
                );
