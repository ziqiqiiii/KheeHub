...
public class MapFragment extends Fragment implements OnMapReadyCallback {
    ...
    // ADDED: Named color constants — replaces all magic hex numbers in updateBottomSheetUI.
    private static final int COLOR_OPEN_TEXT       = 0xFF388E3C;
    private static final int COLOR_OPEN_BG         = 0xFFE8F5E9;
    private static final int COLOR_CLOSED_TEXT     = 0xFFF57C00;
    private static final int COLOR_CLOSED_BG       = 0xFFFFE0B2;
    private static final int COLOR_UNAVAIL_TEXT    = 0xFFD32F2F;
    private static final int COLOR_UNAVAIL_BG      = 0xFFFFEBEE;
    ...
    // CHANGED: ToiletRepository replaces direct Firestore calls — SRP applied.
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        etSearch.setOnItemClickListener((parent, v1, position, id) -> {
            Toilet selected = (Toilet) parent.getItemAtPosition(position);
            if (selected != null) {
                // CHANGED: Using getters instead of direct field access
                mMap.animateCamera(CameraUpdateFactory.newLatLngZoom(
    ...
    private void findNearestToilet() {
        ...
        fusedLocationClient.getLastLocation().addOnSuccessListener(location -> {
            ...
            for (Toilet t : allToilets) {
                boolean matchesTags = true;
                for (String requiredTag : currentSelectedTags) {
                    // CHANGED: Using getTags() and isAvailable() instead of direct field access
                ...
                // CHANGED: isAvailable() replaces status == 0 magic number check
                ...
                float[] results = new float[1];
                // CHANGED: Using getLat()/getLng() getters
                Location.distanceBetween(
                        location.getLatitude(), location.getLongitude(),
            ...
            if (nearest != null) {
                ...
                Toast.makeText(requireContext(),
                        "No nearby toilets match your active filters.", Toast.LENGTH_LONG).show();
            }
        });
    }
    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        // CHANGED: repository.getAllToilets() replaces direct Firestore call
        repository.getAllToilets(
                downloadedToilets -> {
                    ...
                    allMarkers.clear();
                    for (Toilet t : allToilets) {
                        // CHANGED: Using getOpeningHours() getter
                        float markerColor = isCurrentlyOpen(t.getOpeningHours()) ?
                                BitmapDescriptorFactory.HUE_RED :
                                BitmapDescriptorFactory.HUE_ROSE;

                        // CHANGED: Using getLat()/getLng()/getName() getters
                        Marker marker = mMap.addMarker(new MarkerOptions()
                    ...
                    // FIXED: Removed the duplicate moveCamera block that appeared twice before
                    if (!allToilets.isEmpty()) {
                        mMap.moveCamera(CameraUpdateFactory.newLatLngZoom(
                ...
                e -> Toast.makeText(requireContext(),
                        "Failed to load toilets: " + e.getMessage(), Toast.LENGTH_LONG).show()
        );
        ...
        if (requestCode == LOCATION_PERMISSION_REQUEST_CODE) {
            if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                ...
                Toast.makeText(getContext(),
                        "Location permission is needed to show your position.", Toast.LENGTH_SHORT).show();
            }
        }
    }
    ...
    private void searchForToilet(String query) {
        ...
        for (Toilet t : allToilets) {
            // CHANGED: Using getName()/getFloor() getters
    ...
    private void updateBottomSheetUI(Toilet toilet, Float distanceInMeters) {
        currentToilet = toilet;
        // CHANGED: Using getName()/getFloor()/getRating() getters
        ...
        // CHANGED: isAvailable() replaces status == 1 checks.
        // CHANGED: Named color constants replace all magic hex numbers.
        if (toilet.isAvailable() && isCurrentlyOpen(toilet.getOpeningHours())) {
            tvStatus.setText("Open");
            tvStatus.setTextColor(COLOR_OPEN_TEXT);
            tvStatus.setBackgroundColor(COLOR_OPEN_BG);
        } else if (toilet.isAvailable() && !isCurrentlyOpen(toilet.getOpeningHours())) {
            tvStatus.setText("Close");
            tvStatus.setTextColor(COLOR_CLOSED_TEXT);
            tvStatus.setBackgroundColor(COLOR_CLOSED_BG);
        } else {
            tvStatus.setText("Under Construction");
            tvStatus.setTextColor(COLOR_UNAVAIL_TEXT);
            tvStatus.setBackgroundColor(COLOR_UNAVAIL_BG);
        }
    ...
    private void showFilterDialog() {
        ...
        switchOpenNow.setChecked(isFilterOpenNow);
        // CHANGED: ChipUtils.restoreSelectedTags replaces the manual restore loop
        if (cgFilterTags != null) {
        ...
        btnApply.setOnClickListener(v -> {
            isFilterOpenNow = switchOpenNow.isChecked();
            currentSelectedTags.clear();
            // CHANGED: ChipUtils.getSelectedTags replaces the duplicated collection loop
            if (cgFilterTags != null) {
    ...
    private void applyFiltersToMap() {
        ...
        for (Marker marker : allMarkers) {
            ...
            boolean matchesOpen = true;
            if (isFilterOpenNow) {
                // CHANGED: isAvailable() replaces status != 1 check
                if (!t.isAvailable() || !isCurrentlyOpen(t.getOpeningHours())) {
                    matchesOpen = false;
                }
            }
        ...
        Toast.makeText(getContext(), "Found " + visibleCount + " matching toilets",
    ...
    private void showEditDialog(Toilet toilet) {
        ...
        // CHANGED: Using getters to pre-fill values
        ...
        // CHANGED: ChipUtils.restoreSelectedTags replaces the manual tag-restore loop
        ...
        btnSave.setOnClickListener(v -> {
            ...
            // CHANGED: repository.updateToilet() replaces the nested Firestore calls
            repository.updateToilet(
                    ...
                    unused -> {
                        ...
                        // Update local object via setters
